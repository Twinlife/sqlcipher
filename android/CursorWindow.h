...
public:
    ...
    uint32_t            read_field_slot(uint32_t row, uint32_t column, field_slot_t * slot);
    ...
    row_slot_t *        getRowSlot(uint32_t row);

                        /**
                         * return NULL if Failed to find rowSlot or
                         * Invalid rowSlot
                         */
    field_slot_t *      getFieldSlotWithCheck(uint32_t row, uint32_t column);
    field_slot_t *      getFieldSlot(uint32_t row, uint32_t column)
                            {
